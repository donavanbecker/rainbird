...
- `isActive(zone?: number): boolean`
  ...
  const isZone1Active = rainbird.isActive(1)
  ```
...
- `isInUse(zone?: number): boolean`
  ...
  const isZone1Running = rainbird.isInUse(1)
  ```
...
- `remainingDuration(zone?: number): number`
  ...
  const zone1TimeLeft = rainbird.remainingDuration(1)
  ```
...
- `isProgramRunning(programId: string): boolean | undefined`
  ...
  const isProgramArunning = await rainbird.isProgramRunning('A')
  ```
...
- `getIrrigatinDelay(): Promise<number>`
  ...
  const delayDaysLeft = await rainbird.getIrrigatinDelay()
  ```
...
- `version: string`
  ...
  const version = rainbird.version
  ```
...
- `serialNumber: string`
  ...
  const serialNumber = rainbird.serialNumber
  ```
...
- `rainSetPointReached: boolean`
  ...
  const setPointReached = rainbird.rainSetPointReached
  ```
